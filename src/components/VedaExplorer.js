...
const SearchableDropdown = ({ 
  label, value, placeholder, options, onSelect, searchValue, onSearchChange, 
  isOpen, setIsOpen, disabled, dropdownRef, renderOption, helpText 
}) => (
  <div style={{ flex: "1 1 250px" }} ref={dropdownRef}>
        ...
        <div style={{
          ...
          <div style={{ padding: "0.75rem", borderBottom: "1px solid #e5e7eb", background: "#f8fafc" }}>
            ...
            <input type="text" placeholder={`Search...`} value={searchValue}
              ...
              style={{
                width: "100%", padding: "0.5rem 0.5rem 0.5rem 2.5rem", border: "1px solid #d1d5db",
    ...
    {helpText && <small style={{ color: disabled ? "#9ca3af" : "#10b981", fontSize: "0.8rem", display: "block", marginTop: "0.25rem" }}>{helpText}</small>}
  </div>
);
  ...
  <div style={{
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
      ...
      <button onClick={toggleAudio} style={{
        ...
        background: playing ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" : "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)",
    ...
    <div style={{
      ...
      <p style={{
        fontSize: "1.6rem", textAlign: "center", color: "#1e1b4b", lineHeight: "2",
    ...
    <div style={{
      ...
      <p style={{ fontSize: "1.2rem", color: "#1e1b4b", lineHeight: "1.8", fontStyle: "italic", marginTop: "1rem" }}>
        "{sloka.translation}"
      </p>
    </div>
  </div>
...
export default function VedaExplorer() {
  ...
  const [availableMandalas] = useState(Array.from({length: 10}, (_, i) => i + 1));
  ...
  const [inputMode, setInputMode] = useState("guided"); // "guided" or "quick"
  ...
  const handleQuickInput = () => {
    ...
    const [, m, h, s] = match; // Skip first element (full match)
  ...
  return (
    <div style={{ 
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "0.75rem", marginBottom: "2rem" }}>
          <button onClick={() => setInputMode("guided")} 
            style={{ 
              ...
              border: inputMode === "guided" ? "none" : "2px solid #e2e8f0",
              background: inputMode === "guided" ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" : "white",
              color: inputMode === "guided" ? "white" : "#334155", 
              fontWeight: "700", 
              fontSize: "1.05rem",
              cursor: "pointer", 
              transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
              boxShadow: inputMode === "guided" ? "0 10px 30px rgba(102,126,234,0.3)" : "none"
            }}
            onMouseEnter={(e) => {
              if (inputMode !== "guided") {
            ...
            onMouseLeave={(e) => {
              e.target.style.background = inputMode === "guided" ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" : "white";
              e.target.style.borderColor = inputMode === "guided" ? "transparent" : "#e2e8f0";
              e.target.style.transform = "translateY(0)";
              e.target.style.boxShadow = inputMode === "guided" ? "0 10px 30px rgba(102,126,234,0.3)" : "none";
            }}>
            <FaGripHorizontal /> Guided Selection
          </button>
          <button onClick={() => setInputMode("quick")}
            style={{ 
              display: "flex", alignItems: "center", gap: "0.5rem", 
              padding: "1rem 2rem", borderRadius: "50px",
              border: inputMode === "quick" ? "none" : "2px solid #e2e8f0",
              background: inputMode === "quick" ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" : "white",
              color: inputMode === "quick" ? "white" : "#334155", 
              fontWeight: "700", 
              fontSize: "1.05rem",
              cursor: "pointer", 
              transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
              boxShadow: inputMode === "quick" ? "0 10px 30px rgba(102,126,234,0.3)" : "none"
            }}
            onMouseEnter={(e) => {
              if (inputMode !== "quick") {
                e.target.style.background = "#f8fafc";
            ...
            onMouseLeave={(e) => {
              e.target.style.background = inputMode === "quick" ? "linear-gradient(135deg, #667eea 0%, #764ba2 100%)" : "white";
              e.target.style.borderColor = inputMode === "quick" ? "transparent" : "#e2e8f0";
              e.target.style.transform = "translateY(0)";
              e.target.style.boxShadow = inputMode === "quick" ? "0 10px 30px rgba(102,126,234,0.3)" : "none";
            }}>
            <FaBolt /> Quick Input
          </button>
        </div>
        ...
        {inputMode === "guided" ? (
          <>
            ...
            <div style={{ 
              ...
              {/* Mandala Grid Selection */}
              <div style={{ marginBottom: "2rem" }}>
                <label style={{ display: "block", marginBottom: "1rem", fontWeight: "700", color: "#1e1b4b", fontSize: "1.1rem" }}>
                  📚 Step 1: Choose Mandala (Book)
                </label>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "1rem" }}>
                  {availableMandalas.map(n => (
                    <div key={n} onClick={() => { setMandalaNum(n.toString()); setHymnNum(""); setStanzaNum(""); setSloka(null); setError(null); }}
                      style={{ 
                        padding: "1.5rem 1rem", borderRadius: "16px", textAlign: "center", cursor: "pointer",
                        border: mandalaNum === n.toString() ? "3px solid #667eea" : "2px solid #e5e7eb",
                        ...
                        transition: "all 0.3s", boxShadow: mandalaNum === n.toString() ? "0 8px 20px rgba(102, 126, 234, 0.3)" : "0 2px 8px rgba(0,0,0,0.05)"
                      }}>
                      <div style={{ 
                        fontSize: "2rem", fontWeight: "700", marginBottom: "0.5rem",
                        color: mandalaNum === n.toString() ? "#667eea" : "#6b7280"
                      }}>
                        {n}
                      </div>
                ...
                {mandalaNum && indexData && (
                  <p style={{ fontSize: "0.9rem", color: "#10b981", marginTop: "1rem", fontWeight: "600", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <FaCheck /> Contains {indexData.total_hymns} hymns
                  </p>
                )}
              </div>

              {/* Hymn and Stanza Dropdowns */}
              {mandalaNum && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", color: "black" }}>
                  <div>
                    <label style={{ display: "block", marginBottom: "1rem", fontWeight: "700", color: "#1e1b4b", fontSize: "1.1rem" }}>
                      🎵 Step 2: Select Sukta (Hymn)
                    </label>
                    <SearchableDropdown
                      label=""
                      value={hymnNum ? `Sukta ${hymnNum} (${indexData?.hymns?.find(h => h.hymn_number === Number(hymnNum))?.total_stanzas || 0} stanzas)` : ""}
                      placeholder="Search or select sukta..."
                      options={getFilteredHymns()}
                      onSelect={(h) => { setHymnNum(h.hymn_number); setStanzaNum(""); setSloka(null); setIsHymnDropdownOpen(false); }}
                  ...
                  <div>
                    <label style={{ display: "block", marginBottom: "1rem", fontWeight: "700", color: "#1e1b4b", fontSize: "1.1rem" }}>
                      📝 Step 3: Select Mantra (Stanza)
                    </label>
                    <SearchableDropdown
                      label=""
                      value={stanzaNum ? `Mantra ${stanzaNum}` : ""}
                      placeholder="Search or select mantra..."
            ...
            <div style={{ textAlign: "center", marginBottom: "3rem" }}>
              <button onClick={handleSearch} disabled={loading || !mandalaNum || !hymnNum || !stanzaNum}
                style={{ 
                  background: (!mandalaNum || !hymnNum || !stanzaNum) ? "#9ca3af" : "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", 
                  color: "white", border: "none", padding: "1rem 3rem", borderRadius: "50px", fontWeight: "700", fontSize: "1.1rem",
                ...
                onMouseEnter={(e) => {
                  if (mandalaNum && hymnNum && stanzaNum && !loading) {
                    e.target.style.transform = "translateY(-3px)";
                    e.target.style.boxShadow = "0 15px 40px rgba(102,126,234,0.4)";
                  }
                }}
                onMouseLeave={(e) => {
                  e.target.style.transform = "translateY(0)";
                  e.target.style.boxShadow = (!mandalaNum || !hymnNum || !stanzaNum) ? "none" : "0 10px 30px rgba(102,126,234,0.3)";
                }}>
          ...
          <div style={{ 
            ...
            borderRadius: "24px", 
            padding: "2.5rem", 
            boxShadow: "0 15px 50px rgba(245,158,11,0.15)", 
            border: "2px solid rgba(245,158,11,0.25)",
            marginBottom: "3rem",
            backdropFilter: "blur(10px)"
          }}>
            <h3 style={{ fontSize: "1.5rem", fontWeight: "700", color: "#1e1b4b", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <div style={{ display: "flex", gap: "1rem", alignItems: "flex-start" }}>
              <input type="text" value={quickInput} onChange={(e) => setQuickInput(e.target.value)}
                ...
                placeholder="e.g., 1.10.129"
                style={{ flex: 1, padding: "1rem 1.5rem", border: "2px solid #e5e7eb", borderRadius: "16px", 
              ...
              {["1.1.1", "2.5.3", "10.129.1"].map(ex => (
                <button key={ex} onClick={() => setQuickInput(ex)}
                  style={{ padding: "0.5rem 1rem", background: "#f3f4f6", border: "1px solid #e5e7eb", 
                    borderRadius: "8px", cursor: "pointer", color: "#1e1b4b", fontWeight: "600", fontSize: "0.9rem" }}>
                  {ex}
                </button>
              ))}
            </div>
        ...
        {/* Error Message */}
        {error && (
          <div style={{ background: "rgba(239, 68, 68, 0.1)", border: "2px solid rgba(239, 68, 68, 0.3)", borderRadius: "16px", padding: "1rem", color: "#dc2626", textAlign: "center", marginBottom: "2rem", fontWeight: "600" }}>
            ⚠️ {error}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginTop: "1.5rem" }}>
              ...
              {sloka.padas?.length > 0 && (
                <div style={{ background: "white", borderRadius: "24px", padding: "0", boxShadow: "0 10px 40px rgba(0,0,0,0.08)" }}>
                  <button onClick={() => setShowPadasAnalysis(!showPadasAnalysis)} style={{ width: "100%", background: "#f8fafc", border: "none", padding: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderRadius: "24px" }}>
                  ...
                  {showPadasAnalysis && (
                    <div style={{ padding: "1.5rem", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "1rem" }}>
                      {sloka.padas.map((p, i) => (
                        <div key={i} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "1.2rem" }}>
                          <div style={{ fontSize: "1.1rem", fontWeight: "700", marginBottom: "0.8rem", fontFamily: "'Noto Serif Devanagari', serif" }}>{p.form}</div>
                          <div style={{ fontSize: "0.85rem", color: "#64748b", marginBottom: "0.8rem" }}><strong>Lemma:</strong> {p.lemma}</div>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                            {Object.entries(p).filter(([k]) => !['form', 'lemma'].includes(k)).map(([k, v]) => (
                              <span key={k} style={{ background: "#667eea", color: "white", padding: "0.2rem 0.6rem", borderRadius: "12px", fontSize: "0.75rem" }}>{k}: {v}</span>
                            ))}
                          </div>
                        </div>
                      ))}
              ...
              {sloka.translations && Object.keys(sloka.translations).filter(k => k.toLowerCase() !== 'griffith').length > 0 && (
                <div style={{ background: "white", borderRadius: "24px", padding: "0", boxShadow: "0 10px 40px rgba(0,0,0,0.08)" }}>
                  <button onClick={() => setShowAllTranslations(!showAllTranslations)} style={{ width: "100%", background: "#f8fafc", border: "none", padding: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderRadius: "24px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.8rem" }}>
                      <FaQuoteLeft color="#10b981" />
                      <span style={{ fontWeight: "600" }}>Alternative Translations ({Object.keys(sloka.translations).filter(k => k.toLowerCase() !== 'griffith').length})</span>
                    </div>
                    {showAllTranslations ? <FaCompress color="#10b981" /> : <FaExpand color="#10b981" />}
                  </button>
                  {showAllTranslations && (
                    <div style={{ padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                      {Object.entries(sloka.translations).filter(([t]) => t.toLowerCase() !== 'griffith').map(([translator, translation]) => (
                        <div key={translator} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "1.5rem", borderLeft: "4px solid #10b981" }}>
                          <div style={{ fontSize: "0.9rem", fontWeight: "700", color: "#10b981", marginBottom: "0.8rem" }}>{translator.toUpperCase()}</div>
                          <p style={{ fontSize: "1.05rem", color: "#1e293b", lineHeight: "1.6", fontStyle: "italic", margin: 0 }}>"{translation}"</p>
                        </div>
                      ))}
                    </div>
                  )}
